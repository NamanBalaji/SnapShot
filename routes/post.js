...
router.post('/editpost/:postId', requireLogin,(req,res)=>{
    const {title, body} = req.body
    Post.findByIdAndUpdate(req.params.postId, {title:title, body:body},(err,result)=>{
        if(result){
            res.json({message:"Post Edited Successfully"})
        }
        else{
            res.json({message:"Failed to edit"})
        }
    })
})
